		...
		case DXGI_FORMAT_R16G16_FLOAT:
			{
				for ( unsigned iTexel = 0; iTexel < m_imageWidth * m_imageHeight; ++iTexel )
				{
					ReadR16G16_Float( texel[ 0 ], srcPtr[ 0 ] );
					ReadR16G16_Float( texel[ 1 ], srcPtr[ 1 ] );
					AddTexel( texel );
				}
			}
			break;
		...
		case DXGI_FORMAT_R32G32B32_FLOAT:
			{
				for ( unsigned iTexel = 0; iTexel < m_imageWidth * m_imageHeight; ++iTexel )
				{
					ReadR32G32B32_Float( texel[ 0 ], srcPtr[ 0 ] );
					ReadR32G32B32_Float( texel[ 1 ], srcPtr[ 1 ] );
					AddTexel( texel );
				}
			}
			break;
...
void CCompareWindow::PickTexel( QPoint const& pos )
{
	...
	if ( texelX >= 0 && texelX < m_imageWidth && texelY >= 0 && texelY < m_imageHeight )
	{
		...
		switch ( m_info[ 0 ].format )
		{
			...
			case DXGI_FORMAT_R32G32B32_FLOAT:
				{
					TexelInfoR32G32B32_Float( texelInfo0, src0Ptr );
					TexelInfoR32G32B32_Float( texelInfo1, src1Ptr );
				}
				break;
