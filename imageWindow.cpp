		...
		case DXGI_FORMAT_R16G16B16A16_TYPELESS:
			{
				for ( unsigned iTexel = 0; iTexel < m_imageWidth * m_imageHeight; ++iTexel )
				{
					ReadR16G16B16A16_UNorm( texel, srcPtr );
					AddTexel( texel );
				}
			}
			break;
		...
		case DXGI_FORMAT_R32G32B32_FLOAT:
			{
				for ( unsigned iTexel = 0; iTexel < m_imageWidth * m_imageHeight; ++iTexel )
				{
					ReadR32G32B32_Float( texel, srcPtr );
					AddTexel( texel );
				}
			}
			break;
...
void CImageWindow::PickTexel( QPoint const& pos )
{
	...
	if ( texelX >= 0 && texelX < m_imageWidth && texelY >= 0 && texelY < m_imageHeight )
	{
		...
		switch ( m_info.format )
		{
			...
			case DXGI_FORMAT_R16G16B16A16_TYPELESS:
				{
					TexelInfoR16G16B16A16_UNorm( texelInfo, srcPtr );
				}
				break;
			...
			case DXGI_FORMAT_R32G32B32_FLOAT:
				{
					TexelInfoR32G32B32_Float( texelInfo, srcPtr );
				}
				break;
